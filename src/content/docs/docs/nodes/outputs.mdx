...
The material endpoint. Wire base colour, roughness, metallic, normal and height here to drive the 3D preview and the exporter. Every channel has an on/off toggle and, in most cases, a constant fallback, so a material looks sensible long before all its maps are wired.

**Inputs:** Base Color (colour map, optional), albedo. Normal (colour map, optional), tangent-space normal. Roughness (grayscale map, optional). Metallic (grayscale map, optional). Specular (colour map, optional), for the Specular/Glossiness workflow. Glossiness (grayscale map, optional). AO (grayscale map, optional), ambient occlusion. Height (grayscale map, optional). Opacity (grayscale map, optional). Clearcoat (grayscale map, optional). No input is required; a graph renders and exports with any subset connected.
...
| Base (colour) | Colour | 0.8, 0.8, 0.8 | each 0.0 to 1.0 | Constant fallback when Base Color is enabled but nothing is wired. Edited as one colour swatch. |
...
| Specular (colour) | Colour | 0.04, 0.04, 0.04 | each 0.0 to 1.0 | Constant fallback for Specular. Edited as one colour swatch. |
...
**Outputs:** Output (colour map), the composited sheet for the channel chosen by Preview Channel. Export walks all six channels through the same compositor.
...
Only connected slots become strips, and they close up in order: if you connect slots 2, 3 and 4 you get a three-strip sheet, not a blank top strip. A slot whose upstream has not produced anything yet is skipped rather than rendered as a placeholder. Channel neutrals for a strip with no data are a flat normal, white AO, black metallic and mid grey for everything else.

Vertical seams between neighbouring strips are inherent to trim sheets; each strip is its own material band. Within one strip the content still wraps cleanly against itself, and each strip tiles horizontally wherever its source does.
