...
:::caution[Thin Wall is now the Thickness channel]
The 3D preview's Thin Wall toggle is gone, so a preview that relied on it shades differently now. Subsurface scattering is a material channel instead: wire a map into Thickness, or turn the channel on and raise Thickness Value. It exports like any other map. The channel changes nothing on its own (it starts off, with a constant of 0, which scatters nothing), so existing materials render as they did until you use it.
:::
