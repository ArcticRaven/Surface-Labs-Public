...
| Seed | Seed | 0 | Any whole number | Re-rolls Random Value, Random Color and Random Slope's per-region direction. It is applied while the map is drawn, so scrubbing it never re-runs the Flood Fill analysis, and two Region Query nodes on one Flood Fill can carry different randomness. At 0, Random Value and Random Color keep the values the Flood Fill baked, so existing projects render exactly as before; Random Slope's direction is seeded per region either way, since randomizing it is that mode's own job. |
