...
Shows the tonal distribution of its input and lets you remap a slice of it in the same node. Read it to see where your values actually sit; drag its window bars to compress them into a band. Left at defaults it is a pure analyzer and passes the image through unchanged.
