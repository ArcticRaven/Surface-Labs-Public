...
:::caution[Rewire needed from 1.2.1 and earlier]
Height Blend used to carry an Output parameter that switched the single port
between Height and Mask. The parameter is gone. A project that had it set to
Mask now gets the height on its existing wire, so move that consumer to the
Blend Mask port.
:::
