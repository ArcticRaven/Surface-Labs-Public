...
Random button, and color editor.
:::

:::caution[Trap: a vector parameter without @color is not a vector]
The control set has no multi-component widget outside the color editor, so a
...
An initializer may be a plain number, a signed number, `true` / `false`, or a
