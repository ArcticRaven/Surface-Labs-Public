...
`--format` takes the same set the Export dialog offers. There is no JPEG, because
lossy compression damages texture maps, so `--format jpg` is rejected. `tif` is
accepted as a spelling of `tiff`.
